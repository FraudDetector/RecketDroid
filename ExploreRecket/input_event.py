...
class EventLog(object):
    ...
    def __init__(self, device, app, event, profiling_method=None, tag=None):
        ...
        if profiling_method is not None and \
                str(profiling_method) != "full" and str(profiling_method) != "red_packet" and \
    ...
    def start_profiling(self, sign):
        ...
        if pid is None:
            if self.is_start_event() and self.profiling_method != 'red_packet':
        ...
        elif self.profiling_method == 'full' or (self.profiling_method == 'red_packet' and sign == '_red'):
            # track all events or only red packet activation events
    ...
    def stop(self, sign):
        ...
        """
        self.stop_profiling(sign)
        self.to_state = self.device.get_current_state()
        self.save2dir()
        self.save_views()

    def stop_profiling(self, sign, output_dir=None):
        if self.profiling_method is None:
            return
        ...
        try:
            ...
            if output_dir is None:
                if self.device.output_dir is None:
                    ...
                    output_dir = os.path.join(self.device.output_dir, "events/trace")
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            event_trace_local_path = "%s/event_trace_%s%s.trace" % (output_dir, self.tag, sign)
...
class KeyEvent(InputEvent):
    ...
    def get_event_str(self, state):
        if state is not None:
            ...
            return "%s(state= None, name=%s)" % (self.__class__.__name__, self.name)
...
class ScrollEvent(UIEvent):
    ...
    def send(self, device):
        ...
        if self.direction == "UP":
            ...
            start_y += height * 2 / 5
            end_y -= height * 2 / 5
            # print("起始坐标(x,y)：", start_x, start_y)
            # print("结束坐标(x,y)：", end_x, end_y)
