...
from .input_policy import UtgBasedInputPolicy, UtgNaiveSearchPolicy, UtgGreedySearchPolicy, \
    UtgReplayPolicy, ManualPolicy, \
    POLICY_NAIVE_DFS, POLICY_GREEDY_DFS, \
    POLICY_NAIVE_BFS, POLICY_GREEDY_BFS, \
    POLICY_REPLAY, POLICY_MEMORY_GUIDED, \
    POLICY_MANUAL, POLICY_MONKEY, POLICY_NONE, POLICY_RECKET_FIRST
from .input_policy3 import UtgRecketSearchPolicy
...
class InputManager(object):
    ...
    def get_input_policy(self, device, app, master):
        if self.policy_name == POLICY_NONE:
            ...
            input_policy = None
        elif self.policy_name == POLICY_RECKET_FIRST:
            input_policy = UtgRecketSearchPolicy(device, app, self.random_input, self.policy_name)
    ...
    def add_event(self, event, sign=''):
        ...
        event_log.start(sign)
        while True:
            # Add: Prolong launching time if tracking a start event.
            if event.event_type == 'intent' and 'am start --start-profiler' in event.intent:
                interval = self.event_interval + 5
            else:
                interval = self.event_interval
            time.sleep(interval)
